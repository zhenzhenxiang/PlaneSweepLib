 ...
 cv::Mat eulerAnglesToRotationMatrix(const cv::Vec3f theta)
{
  ...
  cv::Mat R_x = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, cos(theta[0]), -sin(theta[0]), 0,
             sin(theta[0]), cos(theta[0]));

  // Calculate rotation about y axis
  cv::Mat R_y = (cv::Mat_<double>(3, 3) << cos(theta[1]), 0, sin(theta[1]), 0, 1, 0,
...
int main(int argc, char* argv[])
{
    ...
    for (unsigned int i = 0; i < numCam; i++)
    {
      ...
      calibrationStr >> xi;
      calibrationStr >> dist_coeff[0] >> dist_coeff[1] >> dist_coeff[2] >> dist_coeff[3];
      ...
      cams.push_back(PSL::FishEyeCameraMatrix<double>(K, R.transpose(), -R.transpose() * C, xi));
...
//    }
    ...
    // Plane sweeping stereo
    {
        ...
        cFEPS.setPlaneGenerationMode(PSL::FISH_EYE_PLANE_SWEEP_PLANEMODE_UNIFORM_DEPTH_GROUND);
...

        {
            ...
            makeOutputFolder("fisheyeTestResultsSaic/grayscaleZNCC/NoOcclusionHandling/");
            cv::imwrite("fisheyeTestResultsSaic/grayscaleZNCC/NoOcclusionHandling/refImg.png",refImage);
            float minDepth = 1.0;
            float maxDepth = 60.0;
            fEDM.saveInvDepthAsColorImage("fisheyeTestResultsSaic/grayscaleZNCC/NoOcclusionHandling/invDepthCol.png", minDepth, maxDepth);
